...

{
    ...
    begin
        ...
        if (ActivationCodeInfo."Data Activated" <= Today) and (Today <= ActivationCodeInfo."Expiration Date") then
            exit(true);
        exit(false);
    end;
    ...
    //var
    // CustomerRewardsWizard: Page "Customer Reward Wizard";
    //begin
        ...
        if (GetHttpResponse(ActivationCode, ResponseText)) then begin
            ...
            if (JsonRepsonse.SelectToken('ActivationResponse', Result)) then begin
                ...
                if (Result.AsValue().AsText() = 'Success') then begin
                    ...
                    ActivationCodeInfo."Data Activated" := Today;
